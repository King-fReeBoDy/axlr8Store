...
function CancelledItems({ cancelled }) {
        ...
        {cancelled.map((cancel) => (
          <div className="my-3 border-b-2 p-2">
            <div className="flex justify-between">
              <p>
                <span>ORDER CODE</span> {cancel.order_ref_code}
              </p>
              <p className="text-gray-400">{cancel.modified.slice(0, 10)}</p>
            </div>

            <div className="">
              {cancel.items.map((item) => {
                return (
            ...
            <div className="flex justify-between">
              <p className="text-gray-400">{cancel.buyer.name}</p>
              <p className="text-sky-500 uppercase">{cancel.order_status}</p>
            </div>
          </div>
        ))}
      </div>
